...
int query_path(int x) {
    int res = 0;
    while (top[x] != top[0]) {
        res = (res + query(1, 1, n, dfn[top[x]], dfn[x], 0)) % mod;
        x = fa[top[x]];
    }
    res = (res + query(1, 1, n, dfn[0], dfn[x], 0)) % mod;
    return res;
}
...
int main() {
    ...
    for (int i = 1; i <= m; i++) {
        cin >> l >> r >> z;
        que.emplace_back(array<int, 3>{l - 1, z, -i});
        que.emplace_back(array<int, 3>{r, z, i});
    }
    int cur = 0;
    vector<int> ans(m + 1);
    ...
    for (int i = 0; i < que.size(); i++) {
        ...
        if (que[i][2] < 0) {
            ans[-que[i][2]] -= query_path(que[i][1]);
        } else {
            ans[que[i][2]] += query_path(que[i][1]);
        }
    }
