...
// 判断点在多边形内
int in_polygon(const Point &p, const vector<Point> &poly) {
    int flag = 0;
    for(int i = 0; i < poly.size(); i++) {
        Point p1 = poly[i];
        Point p2 = poly[(i + 1) % poly.size()];
        if(on_seg(p, p1, p2)) return 3; // 在边界上
    ...
    if (flag == 0) return 2; // 在多边形外
    if (flag != 0) return 1; // 在多边形内
}
