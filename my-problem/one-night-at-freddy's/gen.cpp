...
int main() {
    ...
    for (int i = 1; i <= t; i++) {
        vector<int> test;
        uniform_int_distribution<int> dist7(1, 100);
        int m = dist7(rnd);
        uniform_int_distribution<int> dist6(1, 100);
        int l = 100;
        uniform_int_distribution<int> dist4(1, 30);
        int n = dist4(rnd);
        fout << n << " " << m << " " << l << "\n";
        int x = 0;
        for (int j = 1; j <= n; j++) {
            uniform_int_distribution<int> dist5(x + 1, l / 2 + j);
            x = dist5(rnd);
            fout << x;
            if (j < n) {
                fout << " ";
