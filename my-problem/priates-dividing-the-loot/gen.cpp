...
int main() {
    ...
    uniform_int_distribution<int> dist2(11, 20000);
    uniform_int_distribution<int> dist3(11, 1000000000);
    int n = 10;
    ...
    pair<int, int> test;
    for (int i = 0; i < n; i++) {
        test.first = dist2(rnd);
        test.second = 1000000000;
        tests.push_back(test);
    }
    
    fout << tests.size() << "\n";
    for (auto &test : tests) {
        fout << test.first << " " << test.second;
        fout << "\n";
    }
    fout.close();
